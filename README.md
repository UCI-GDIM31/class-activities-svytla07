...
### W7
[Game Idea](https://docs.google.com/document/d/13nVwuWGt47ZJIsqeRlTBOlD78hPmrMBLpVFdvgc_doA/edit?usp=sharing) . My role in the group was game Physics. Notes: In order to code the movement we need to mutiply the input by the speed and the time that has passed, TimeDeltatime. The code in Step 2 was wrong because it was usiing a vector component to try and move the object, which would cause it to move forward along the world space, not forward in it's own direction, which could be done by using transform. 
## Open-Source Assets
### W1
